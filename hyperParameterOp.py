...
# trying to load Bayesian optimizer package (smart search, best)
# if it doesnt exist then try BOHB (smart search with early stopping of bad trials)
# othw as default use random search (dumb)
# other options: Grid search (predictable but scales terribly as the ranges grow)

# For optuna installation
# Pip: Run "pip install optuna" 
# or
# Conda: Run "conda install -c conda-forge optuna" 

# For BOHB installation
...
task = Task.init(project_name='Hyper-Parameter Optimization',
                 task_name='Demo Hyper-Parameter Optimization',
...
if not args.template_task_id:
    args.template_task_id = Task.get_task(
        project_name='yolov8', task_name='training').id
...
an_optimizer = HyperParameterOptimizer(
    ...
    hyper_parameters=[
        UniformIntegerParameterRange('General/patience', min_value=50, max_value=150, step_size=50),
        UniformIntegerParameterRange('General/imgsz', min_value=320, max_value=640, step_size=160),
        DiscreteParameterRange('General/batch_size', values=[4, 8, 16]),
        DiscreteParameterRange('General/epochs', values=[3, 4, 5]),
    ],
    # this is the objective metric we want to maximize/minimize
    objective_metric_title='mAP50(B)',
    objective_metric_series='metrics',
